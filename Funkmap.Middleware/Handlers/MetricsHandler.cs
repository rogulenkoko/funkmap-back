...
namespace Funkmap.Middleware.Handlers
{
    public class MetricsHandler : DelegatingHandler
    {
        protected override Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
        {
            ...
            var msRoutesKey = "MS_SubRoutes";
            if(!routeData.Values.ContainsKey(msRoutesKey)) return base.SendAsync(request, cancellationToken);
            var subRoutes = routeData.Values[msRoutesKey] as IHttpRouteData[];
